#!/usr/bin/env python3
"""
generalised_brd_generator.py (v5 — FR dict-shape fix)

Fixes vs v4:
  Issue FR — Functional Requirements: each req item may be a dict with
             {Requirement, Priority, trace} keys (from brd_llm.py), not a
             plain string. Parser now extracts Requirement/Priority fields
             from the dict before building the reqs list.
...
def _get_section(data: dict, key: str, fallback_parent: str = None) -> dict:
    """
    Get a top-level section. If not found at top level AND fallback_parent
    is given, also check inside that parent. This handles the model sometimes
    nesting '0. Document Revisions' inside '0. Project Details'.
    """
    if key in data:
        return data[key]
...
def parse_brd(data: dict) -> dict:
    ...
    # Fix 3: check both top-level AND inside Project Details as fallback
    ...
    # ── Document Revisions (Fix 3: fallback-aware) ──────────────────────────
    ...
    # ── Stakeholder Approvals (Fix 3: fallback-aware) ───────────────────────
    ...
    # Fix 8: phases — carry (label, description_or_why, has_value) tuples
    ...
    # Fix 2: 3.3 Risks — read flat value array [{Risk, Mitigation}]
    ...
    if isinstance(raw_risks_val, list):
        for r in raw_risks_val:
            if isinstance(r, dict):
                ...
                # Handle nested {value: ...} shape just in case
                if isinstance(risk_text, dict):
                    risk_text = risk_text.get("value", "")
                if isinstance(mit_text, dict):
                    mit_text = mit_text.get("value", "")
                ctx["risks"].append([safe_str(risk_text), safe_str(mit_text)])
    elif isinstance(raw_risks_val, dict):
        # Residual items dict shape not caught by brd_llm.py normalizer
    ...
    # ── KPIs — read flat value array [{Metric, Target}] ────────────────────
    ...
    # ── Functional Requirements (Fix FR: handle dict shape per req item) ────
    ctx["fr_sections"] = []
    for sec_key in sorted(fr_key for fr_key in FR.keys() if fr_key.startswith("6.")):
        sec_obj = FR[sec_key]
        raw_reqs = v(sec_obj) if isinstance(sec_obj, dict) else sec_obj
        sec_why = _why_null_message(sec_obj) if isinstance(sec_obj, dict) else WHY_NULL_FALLBACK
        reqs = []
        if isinstance(raw_reqs, list):
            # Build a short prefix from the section key, e.g. "6.1" → "F01", "6.2" → "F02"
            try:
            ...
            for idx, req in enumerate(raw_reqs, 1):
                ...
                # Fix FR: req may be a dict {Requirement, Priority, trace}
                if isinstance(req, dict):
                    req_text = req.get("Requirement") or req.get("requirement") or ""
                    ...
                    # Normalise priority value — strip surrounding brackets/parens
                    priority = str(priority).strip("[]() ")
                    if priority not in ("P1", "P2", "P3"):
                        priority = "P1"
                    clean = str(req_text).rstrip(".")
                else:
                    # Fallback: plain string with optional [P1]/[P2]/[P3] tag
                    req_str = str(req)
                    priority = "P1"
                    for tag in ["P1", "P2", "P3"]:
...
def build_js(ctx: dict) -> str:
    ...
    # Fix 8: phases as [{label, desc, hasValue}] objects
    phases_js_parts = []
...
// Fix 8: Phasing plan — render nullNote in desc cell for null phases
...
def ensure_docx_package(script_dir: Path):
    ...
    result = subprocess.run([npm_cmd, "install", "docx", "--prefix", str(script_dir)],
...
def main():
    parser = argparse.ArgumentParser(description="Generate BRD Word document from structured JSON.")
    parser.add_argument("input_json", help="Path to BRD JSON file")
    parser.add_argument("output_docx", nargs="?", help="Output .docx path (default: <stem>_BRD.docx)")
    ...
    output_path = (Path(args.output_docx).resolve() if args.output_docx
    ...
    result = subprocess.run([node_cmd, str(tmp_js), str(output_path)],
...
# #!/usr/bin/env python3
# """
# generalised_brd_generator.py (v4 — structural fixes)

# Fixes vs v3:
#   Issue 2 — 3.3 Risks: parser now reads value as flat [{Risk, Mitigation}] array.
#   Issue 3 — Revisions/Approvals: parser checks BOTH top-level AND inside
#              '0. Project Details', so it works regardless of where the model
#              put them (post-processor in brd_llm.py hoists them, but this
#              is a belt-and-suspenders fallback).
#   Issue 8 — Phase 3: if value is null, renders a nullNote using why_null
#              instead of an empty cell in the phasing table.
